...
logger.info("Sesión Spark iniciada correctamente")
...
def calculate_ema(df, column_name, periods_list):
    ...
    for n in periods_list:
        ...
        df = df.withColumn(f"EMA_{n}_{column_name}", 
                         ...
                         .when(col(f"EMA_{n}_{column_name}_prev").isNull(), col(f"EMA_{n}_{column_name}_init"))
                         .otherwise(
...
# Función principal para procesar archivos - ajustada para tu estructura de carpetas
    ...
    for crypto in crypto_dirs:
        ...
        for root, year_dirs, files in walk(crypto_path):
            for year_dir in [d for d in year_dirs if not d.startswith('.')]:
                ...
                logger.info(f"Procesando año: {year_dir} en {year_path}")
                
                # Procesar archivos CSV en el directorio de año
                ...
                for file in csv_files:
                    ...
                    try:
                        ...
                        logger.info(f"Archivo leído correctamente. Filas: {row_count}, Columnas: {col_count}")
                        ...
                        # Renombrar columnas si existen con diferente capitalización
                        ...
                        # Verificar columnas requeridas después de renombrar
                        ...
                        # Asegurar que existe una columna symbol si no está presente
                        if "symbol" not in df.columns:
                            symbol_value = f"CRYPTO:{crypto.upper()}USD"
                            df = df.withColumn("symbol", lit(symbol_value))
                            logger.info(f"Columna 'symbol' añadida con valor: {symbol_value}")
                        
                        # Agregar número de fila para cálculos consecutivos
                        window_spec = Window.partitionBy("symbol").orderBy("date")
                        ...
                        # Eliminar columna temporal de número de fila
                        df = df.drop("row_num")
                        ...
                        # Si el problema persiste, verifica si df está vacío antes de guardar:
                        if df.count() > 0:
                            df.write.mode("overwrite").csv(output_file, header=True)
                        else:
                            logger.warning(f"DataFrame vacío para {file_path}. No se guardará.")
